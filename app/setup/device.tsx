...
export default function SetupDeviceScreen() {
  ...
  const registerMutation = trpc.devices.register.useMutation({
    onSuccess: () => {
      // Always proceed to next step (no pending restriction)
      router.replace("/setup/biometric" as any);
    },
    onError: (err) => {
      setError(err.message || "裝置登錄失敗");
    },
  });
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <View className="flex-1 px-6 pt-8">
        ...
        <Text className="text-2xl font-bold text-foreground mb-2">登錄打卡裝置</Text>
        <Text className="text-muted mb-8">
          系統將記錄此裝置資訊，管理員可在裝置管理頁面查看所有已登錄的裝置。
        </Text>
